...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);
	...
	INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(BITS(Mr(src1 + imm, 1),7,0),32));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
	...
	INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (int)((long long)src1 * (long long)src2));
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (int)(((long long)src1 * (long long)src2) >> 32));
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (uint32_t)(((long long)src1 * (uint64_t)src2) >> 32));
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (uint32_t)(((uint64_t)src1 * (uint64_t)src2) >> 32));
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int)((int)src1 / (int)src2));
