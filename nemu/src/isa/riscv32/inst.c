...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
	...
	INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = BITS((long long)src1 * (long long)src2,31,0));
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = BITS((long long)src1 * (long long)src2,63,32));
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = BITS((long long)src1 * (uint64_t)src2,63,32));
